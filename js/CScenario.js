    ...
    this._createGoal = function () {
        ...
        */
        ...
        _oPoleGoalShapeUp = new CANNON.Cylinder(POLE_UP_SIZE.radius_top, POLE_UP_SIZE.radius_bottom,
                POLE_UP_SIZE.height+1, POLE_UP_SIZE.segments);
        ...
        _oCrossbarBody = new CANNON.Body({mass: 0});
        _oCrossbarBody.addShape(_oPoleGoalShapeUp, new CANNON.Vec3(0, 0, POLE_RIGHT_LEFT_SIZE.height * 0.5));
        _oCrossbarBody.position.set(BACK_WALL_GOAL_POSITION.x, BACK_WALL_GOAL_POSITION.y - UP_WALL_GOAL_SIZE.depth, BACK_WALL_GOAL_POSITION.z);

        _oCrossbarBody.addEventListener("collide", function (e) {
            s_oScenario.poleCollision();
        });

        _oWorld.addBody(_oGoalBodyPoleLeftRight);
        _oWorld.addBody(_oCrossbarBody);
        if (SHOW_3D_RENDER) {
            ...
            var oGoalMaterial = new THREE.MeshPhongMaterial({color: 0xffff00, specular: 0x111111, shininess: 50});
            _oDemo.addVisual(_oCrossbarBody, oGoalMaterial);
        }
    };
