function CStakePanel(oParentContainer) {
  ...
  this._init = function () {
    ...
    _oStakeBox.graphics.beginFill(TEXT_COLOR_1).drawRoundRect(-100, 170, 200, 50, 25);
    ...
    _oIncreaseBox.graphics.beginFill(TEXT_COLOR_1).drawRoundRect(-300, 240, 240, 50, 25);
    ...
    let _oIncreaseText = new CCTLText(_oContainer, -125, 225, 80, 80, 60, "center", TEXT_COLOR_2, GAME_FONT, 1, 0, 0, "+", true, true, true, false);
    ...
    let _oDecreaseText = new CCTLText(_oContainer, -315, 220, 80, 80, 80, "center", TEXT_COLOR_2, GAME_FONT, 1, 0, 0, "-", true, true, true, false);

    _oPlaceStake = new createjs.Shape();
    _oPlaceStake.graphics.beginFill(TEXT_COLOR_2).drawRoundRect(100, 240, 200, 140, 25);
    ...
    for (let i = 0; i < 8; i++) {
      if (i >= 4) {
        _stakeTexts[i] = this.createTextButton(_stakes[i], -310 + 40 * (i - 4) + 50 * (i - 4), 340, 50 + 50 * (i - 4) * 0.5, 50, 25, _stakes[i], "#000000");
      } else {
        _stakeTexts[i] = this.createTextButton(_stakes[i], -310 + 40 * i + 50 * i, 300, 50 + 50 * i * 0.5, 50, 25, _stakes[i], "#000000");
      }
    }
  };
  ...
  this.createTextButton = function (text, x, y, width, height, fontSize, stake, color) {
